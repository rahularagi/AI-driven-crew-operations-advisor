...
def test_get_leg_status_found():
    ...
    with patch("crew_ops.conversation.tools.get_flight_leg", return_value=mock_leg):
        result = get_leg_status("L-001")
    assert result["leg_id"] == "L-001"

def test_get_leg_status_not_found():
    with patch("crew_ops.conversation.tools.get_flight_leg", return_value=None):
...
def test_get_crew_ftl_found():
    ...
    with patch("crew_ops.conversation.tools.get_crew_duty_state", return_value=mock_ftl):
        result = get_crew_ftl("C-001")
    assert result["status"] == "AVAILABLE"

def test_get_crew_ftl_not_found():
    with patch("crew_ops.conversation.tools.get_crew_duty_state", return_value=None):
...
def test_simulate_crew_removal_delegates():
    ...
    with patch("crew_ops.conversation.tools.SimulationService") as MockSvc:
...
def test_action_mark_crew_unavailable_publishes_event():
    ...
    published = []
    with patch("crew_ops.conversation.tools.get_flight_leg", return_value=mock_leg), \
         patch("crew_ops.conversation.tools.get_crew_member", return_value=mock_crew), \
         patch("crew_ops.conversation.tools.event_bus") as mock_bus:
...
def test_action_reassign_crew_legality_fail():
    ...
    with patch("crew_ops.conversation.tools.get_flight_leg", return_value=mock_leg), \
         patch("crew_ops.conversation.tools.get_crew_member", return_value=mock_crew), \
         patch("crew_ops.conversation.tools.get_crew_duty_state", return_value=mock_ftl), \
         patch("crew_ops.conversation.tools.get_licenses_for_crew_member", return_value=[]), \
         patch("crew_ops.conversation.tools.get_leave_records_for_crew", return_value=[]), \
         patch("crew_ops.conversation.tools.check_legality", return_value=(False, "FTL_UNAVAILABLE")):
...
def test_action_reassign_crew_success():
    ...
    with patch("crew_ops.conversation.tools.get_flight_leg", return_value=mock_leg), \
         ...
         patch("crew_ops.conversation.tools.get_licenses_for_crew_member", return_value=[]), \
         patch("crew_ops.conversation.tools.get_leave_records_for_crew", return_value=[]), \
         patch("crew_ops.conversation.tools.check_legality", return_value=(True, None)), \
         patch("crew_ops.conversation.tools.SessionLocal") as mock_sl, \
         patch("crew_ops.conversation.tools.roster_repository") as mock_repo, \
         patch("crew_ops.conversation.tools.event_bus"):
